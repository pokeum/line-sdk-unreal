package co.pokeum.linesdk.unrealwrapper.activity

import android.app.Activity
import android.content.Intent
import android.os.Bundle
import android.util.Log
import co.pokeum.linesdk.unrealwrapper.CallbackPayload
...
import co.pokeum.linesdk.unrealwrapper.model.LoginResultForUnreal
...
class LineSdkWrapperActivity : Activity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode != RESULT_OK || data == null) {
            val errorForUnity = ErrorForUnreal(-1, "login error")
        ...
        when (result.responseCode) {
            LineApiResponseCode.SUCCESS -> {
                ...
                val resultJsonString = gson.toJson(LoginResultForUnreal.convertLineResult(result))
    ...
    private fun parseBotPrompt(name: String?): LineAuthenticationParams.BotPrompt? {
        ...
        return LineAuthenticationParams.BotPrompt.entries.firstOrNull {
