...
public:
	/**
	 * Gets the access token obtained by the login process.
	 */
	/** @cond Doxygen_Suppress_Param */
	UFUNCTION(BlueprintCallable, Category = "Line|Login Result")
	...
	/**
	 * Gets the scopes bound to the `UAccessToken` object by the authorization process.
	 */
	/** @cond Doxygen_Suppress_Param */
	UFUNCTION(BlueprintCallable, Category = "Line|Login Result")
	...
	/**
	 * Gets the user profile, which includes user ID, display name, and so on.
	...
	/**
	 * Gets the friendship status between the user and the LINE Official Account, which may have changed during the login process.
	 * 
	...
	/**
	 * Gets the `nonce` value when requesting an ID token during the login process.
	 * 
